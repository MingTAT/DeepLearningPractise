...
class Layer:
  ...
  def doForward(self, _input):
    ...
    self.input = _input
    self.Z = self.W.dot(_input) + self.b
    self._output = self.NL.forward(self.Z)
    return self._output
  ...
  def doBackward(self, dOutput):
    ...
    #self.dZ = self.NL.backward(self.Z) * dOutput
    """
    ...
    """
    self.dZ = self.NL.backward(dOutput)
    self.dW = self.dZ.dot(self.input.T)
    self.db = np.sum(self.dZ, axis = 1, keepdims=True)
    return self.dZ.T.dot(self.W).T
...
class NeuralNetwork:
  ...
  def print(self, want=['W', 'dW', 'b', 'db', 'Z', 'dZ', 'Input', 'Output', 'dInput', 'dOutput']):
    for l in range(self.nLayers):
...
class ObjectiveFunction:
  def crossEntropyLogitForward(self, logits, y):
    ...
    self.logp = logitTrans - np.log(np.sum(expLogit, axis=0))
    self.numInstance = y.shape[1]
    return - np.sum(y * self.logp) / self.numInstance
